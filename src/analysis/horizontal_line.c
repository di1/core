...
horizontal_line_analysis (struct chart *cht, size_t num_candles,
  ...
  for (int i = (int)(num_candles)-2; i >= 0; --i)
    {
      ...
      if (direction == DIRECTION_RESISTANCE)
        {
          if (cur_candle_close > compare_price)
            break;
        }
      ...
      if (cur_candle_volume > 0
          && (cur_candle_high == compare_price
              || cur_candle_open == compare_price
...
enum RISKI_ERROR_CODE
find_horizontal_line (struct chart *cht, size_t num_candles)
{
  PTR_CHECK (cht, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
