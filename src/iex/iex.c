 ...
 */
enum RISKI_ERROR_CODE
symbol_sanitize (char *s, size_t n)
{
  ...
  TRACE (logger_error (RISKI_ERROR_CODE_INSUFFITIENT_SPACE, __func__,
                       __FILENAME__, __LINE__, "%.*s must have (nil) appened",
                       n, s));
 ...
 */
enum RISKI_ERROR_CODE
print_iex_tp_header (struct iex_tp_header *header)
{
...
bool
is_iex_traffic (char *ip_src, char *ip_dst)
{
...
void
packet_handler (unsigned char *userData, const struct pcap_pkthdr *pkthdr,
                const unsigned char *packet)
{
 ...
 */
enum RISKI_ERROR_CODE
parse_system_event_message (void *payload)
{
  ...
  switch (((struct iex_system_event_message *)payload)->system_event)
    ...
    case START_OF_MESSAGES:
      TRACE (
          logger_info (__func__, __FILENAME__, __LINE__, "start of messages"));
      break;
    ...
    case END_OF_MESSAGES:
      TRACE (
          logger_info (__func__, __FILENAME__, __LINE__, "end of messages"));
      break;
    }
  return RISKI_ERROR_CODE_NONE;
}
 ...
 */
enum RISKI_ERROR_CODE
parse_security_directory_message (void *payload)
{
  PTR_CHECK (payload, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
  ...
  TRACE (logger_info (__func__, __FILENAME__, __LINE__,
                      "security directory message for %s",
                      payload_data->symbol));
 ...
 */
enum RISKI_ERROR_CODE
parse_trading_status_message (void *payload)
{
  PTR_CHECK (payload, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
 ...
 */
enum RISKI_ERROR_CODE
parse_operational_hault_status_message (void *payload)
{
  ...
  TRACE (logger_info (__func__, __FILENAME__, __LINE__,
                      "operation hault message for %.8s",
                      payload_data->symbol));
...
enum RISKI_ERROR_CODE
parse_short_sale_price_test_status_message (void *payload)
{
  PTR_CHECK (payload, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
...
enum RISKI_ERROR_CODE
parse_security_event_message (void *payload)
{
  PTR_CHECK (payload, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
  ...
  switch (payload_data->security_event)
    ...
    case OPENING_PROCESS_COMPLETE:
      TRACE (logger_info (__func__, __FILENAME__, __LINE__,
                          "opening process complete %.8s",
                          payload_data->symbol));
      break;
    case CLOSING_PROCESS_COMPLETE:
      TRACE (logger_info (__func__, __FILENAME__, __LINE__,
                          "closing process complete %.8s",
                          payload_data->symbol));
      break;
    default:
      TRACE (logger_error (
          RISKI_ERROR_CODE_UNKNOWN, __func__, __FILENAME__, __LINE__,
 ...
 */
enum RISKI_ERROR_CODE
parse_price_level_update_message (iex_byte_t side, void *payload)
{
  PTR_CHECK (payload, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
 ...
 */
enum RISKI_ERROR_CODE
parse_trade_report_message (void *payload)
{
  PTR_CHECK (payload, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
 ...
 */
enum RISKI_ERROR_CODE
parse_official_price_message (void *payload)
{
  PTR_CHECK (payload, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
 ...
 */
enum RISKI_ERROR_CODE
parse_trade_break_message (void *payload)
{
  PTR_CHECK (payload, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
...
enum RISKI_ERROR_CODE
parse_auction_information_message (void *payload)
{
  PTR_CHECK (payload, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
 ...
 */
enum RISKI_ERROR_CODE
iex_tp_handler (unsigned char *data)
{
  ...
  for (iex_short_t i = 0; i < header->message_count; ++i)
    {
      ...
      switch (message_header->message_type)
        ...
        case PRICE_LEVEL_UPDATE_SELL_MESSAGE:
          TRACE (parse_price_level_update_message (
              message_header->message_type, payload_body));
