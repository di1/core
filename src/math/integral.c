...
integral_chart (struct chart *cht, size_t a, size_t b, double *res)
{
  PTR_CHECK (cht, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
  PTR_CHECK (res, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);

  COMPARISON_CHECK (a, b, <, RISKI_ERROR_CODE_COMPARISON_FAIL,
                    RISKI_ERROR_TEXT);
  ...
  for (size_t i = a + 1; i <= b - 1; ++i)
    {
      ...
      if (summation <= 0)
        {
          printf (".....");
          COMPARISON_CHECK (summation, 0, <=, RISKI_ERROR_CODE_COMPARISON_FAIL,
                            RISKI_ERROR_TEXT);
        }
    }
...
enum RISKI_ERROR_CODE
integral_line (struct linear_equation *eq, size_t a, size_t b, double *res)
{
...
enum RISKI_ERROR_CODE
integral_const (size_t c, size_t a, size_t b, double *res)
{
  PTR_CHECK (res, RISKI_ERROR_CODE_NULL_PTR, RISKI_ERROR_TEXT);
  COMPARISON_CHECK (a, b, <, RISKI_ERROR_CODE_COMPARISON_FAIL,
                    RISKI_ERROR_TEXT);
