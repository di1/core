...
void
remove_lvl (struct level **lvls, int64_t index, int64_t count)
{
...
void
insert_at (struct level **lvls, int64_t len, int64_t at, struct level lvl)
{
  len += 1;

  // create a new array
  struct level *new_lvl
      = (struct level *)(malloc (len * sizeof (struct level)));
...
void
book_update (bool side, struct book *t, int64_t price, int64_t quantity)
{
  /*
