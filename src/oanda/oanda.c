...
oanda_connect (int *ret)
{
  ...
  if (connect (sockfd, (struct sockaddr *)&servaddr, sizeof (servaddr)) != 0)
    ...
    {
      logger_info (__func__, __FILENAME__, __LINE__,
...
void
SSL_read_line (SSL *conn, char **buff)
{
...
void
SSL_read_http_header (SSL *conn, size_t *content_length)
{
  char *line = NULL;
  while (1)
    {
...
void
SSL_http_request_json (SSL *conn, char *request_body, cJSON **response)
{
  ...
  while (total_read < content_length)
    {
      total_read
          += SSL_read (conn, &response_body[total_read], content_length);
    }
...
enum RISKI_ERROR_CODE
oanda_main_loop (SSL *conn, char *pricing_request_body)
{
  ...
  while (1)
    {
      ...
      size_t num_prices = 0;
      const cJSON *prices_json
          = cJSON_GetObjectItem (price_update_json, "prices");
      num_prices = cJSON_GetArraySize (prices_json);

      for (size_t i = 0; i < num_prices; ++i)
        {
          const cJSON *instrument_price_data
              = cJSON_GetArrayItem (prices_json, i);
          ...
          if (num_bids != 0)
            {
              ...
              cJSON *bid_object
                  = cJSON_GetObjectItem (instrument_best_bid, "price");
...
oanda_live (char *token)
{
  ...
  SSL_http_request_json (conn, http_get_account_instruments,
                         &instruments_json);
  ...
  oanda_tradeble_instruments
      = (char **)malloc (num_instruments * sizeof (char *));
  ...
  for (size_t i = 0; i < num_instruments; ++i)
    {
      ...
      oanda_tradeble_instruments[i]
          = strdup (cJSON_GetStringValue (displayName));
